...
class Config:
    # AI 配置
    AI_PROVIDER = os.getenv("AI_PROVIDER", "openai")  # 可选：kimi、openai、gemini；默认优先使用 OpenAI 中转
    ...
    # Kimi 模型会根据文本长度自动选择：
    # - moonshot-v1-8k：短文本（< 2000 字符）
    # - moonshot-v1-32k：长文本（>= 2000 字符）
    KIMI_BASE_URL = "https://api.moonshot.cn/v1"

    # OpenAI / OpenAI 兼容中转
    OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")
    OPENAI_MODEL = os.getenv("OPENAI_MODEL", "gpt-5.4")
    OPENAI_BASE_URL = os.getenv("OPENAI_BASE_URL", "https://code.rayinai.com/v1")  # 支持兼容 OpenAI 的中转 API
    ...
    SCHEDULER_CHECK_INTERVAL = int(os.getenv("SCHEDULER_CHECK_INTERVAL", "300"))  # 默认每 5 分钟检查一次
